...
var handleErrorFactory = function(logger) {
  var log = logger.log;
  ...
  return function(obj) {
      ...
      if (obj.res) {
          if (obj.error !== undefined) {
              obj.res.send(obj.statusCode || 400, ({result:'error',message:obj.error.message}));
          }
      }
  }
};

exports.middleware = function(config, getSecret, logger) {
  var log = logger.log;
  var handleError = handleErrorFactory(logger);
  ...
  return function (req, res, next) {
    ...
    getSecret({blobId:req.query.signature_blob_id,res:res}, function (err, secret) {
      ...
      //log("provided signature   :" + req.query.signature);
      //log("Correct signature is :" + signature);
