    ...
    q.series([
        function(lib) {
            knex.schema.hasTable('blob').then(function(exists) {
                if (!exists) 
                    ...
                    .then(function () {
                        log('blob table is created in database blobvault');
                        lib.done();
                    });
                else 
                    lib.done();
        ...
        function(lib) {
            knex.schema.hasTable('blob_patches').then(function(exists) {
                if (!exists) 
                    ...
                    .then(function () {
                        log('blob_patches table is created in database blobvault');
                        lib.done();
                    });
                else 
                    lib.done()
        ...
        function(lib) {
            ...
            knex.schema.hasTable('log').then(function(exists) {
                if (!exists) 
                    ...
                    .then(function () {
                        log('log table is created in database blobvault');
                        lib.done()
                    });
                else 
                    lib.done()
        ...
        function(lib) {
            knex.schema.hasColumn('blob', 'encrypted_secret').then(function(exists) {
                log("encrypted_Secret exists:",exists);
                if (!exists) 
                    knex.schema.table('blob', function (table) {
                        table.string('encrypted_secret');
                    }).then(function() {
                        log('added encrypted_secret');
                        lib.done();
                    })
                else 
                    lib.done();
